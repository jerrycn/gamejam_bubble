...
//            return gradientColor
    ...
    private var path: CGPath {
        ...
        case let .polygon(_, sides, _, _):
            return UIBezierPath(roundedRect: CGRect(origin: origin, size: size), cornerRadius: CGFloat(sides)).cgPath
//        case let .star(_, points, sharpness, _, _):
////            return UIBezierPath.cgPath。 bug
//
        }
    }
...
class Graphic: MessageControl {
    ...
    func setOnTouchMovedHandler(_ handler: @escaping ((Touch) -> Void)) {
     ...
     */
    var allowsTouchInteraction: Bool = true {
        didSet {
            guard !suppressMessageSending else { return }
    ...
    init(id: String, graphicType: GraphicType = .graphic, name: String = "") {
        self.id = id
        self.name = name
        self.graphicType = graphicType
    ...
    convenience init(image: Image, name: String = "") {
        if name == "" {
    ...
    convenience init(shape: Shape, color: Color, gradientColor: Color? = nil, name: String = "") {
        if name == "" {
            self.init(graphicType: .graphic, name: "graphic" + String(Graphic.defaultNameCount))
            Graphic.defaultNameCount += 1
        } else {
    ...
    func updateShape(shape: Shape, color: Color, gradientColor: Color) {
        ...
        case .star(radius: _, points: _, sharpness: _): break
...
//        self.shape = basicShape
    ...
    var isAudioPositional: Bool = true {
        didSet {
            guard !suppressMessageSending else { return }
    ...
    var position: Point = Point(x: 0, y: 0) {
        ...
        didSet {
            ...
            guard !suppressMessageSending else { return }
            send(.move(to: CGPoint(x:position.x,y:position.y), duration: 0), withKey: "moveTo")
        }
    }
    ...
    func setTintColor(_ color: UIColor?, blend: Double = 0.5) {
    ...
    func run(_ action: SKAction, key: String? = nil) {
    ...
    func moveBy(x: Double, y: Double, duration: Double) {
    ...
    func move(to: Point, duration: Double) {
        
        let moveAction = SKAction.move(to: CGPoint(x:to.x,y:to.y), duration: duration)
        moveAction.timingMode = .linear
    ...
    func rotate(byAngle angle: Double, duration: Double) {
    ...
    func rotate(toAngle angle: Double, duration: Double) {
    ...
    func scale(to: Double, duration: Double) {
    ...
    func scale(by value: Double, duration: Double) {
        guard !suppressMessageSending else { return }
    ...
    func scaleX(by xScale: Double, y yScale: Double, duration: Double) {
        guard !suppressMessageSending else { return }
    ...
    func scaleX(to xScale: Double, y yScale: Double, duration: Double) {
    ...
    func fadeOut(after seconds: Double) {
    ...
    func fadeIn(after seconds: Double) {
    ...
    func fadeAlpha(to value: Double, duration: Double) {
    ...
    func fadeAlpha(by value: Double, duration: Double) {
    ...
    func orbit(x: Double, y: Double, period: Double = 4.0) {
...
//        send(.pulsate(period: period, count: count), withKey: "pulsate")
    ...
    func shake(duration: Double = 2.0) {
...
//    func addAudio(_ sound: Sound, positional: Bool = true, looping: Bool = true, volume: Double = 100) {
//        hasAudio = true
...
//    }
    ...
    convenience init(_ image: Image) { self.init() }
}
